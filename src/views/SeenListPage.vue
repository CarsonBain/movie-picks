<template>
  <div class="mt-8 md:mt-12 px-6 md:px-16">
    ...
    <section class="mt-6">
      <div class="mt-8">
        <div class="flex flex-col items-center mx-auto mt-16 max-w-lg" v-if="!seenMovies.length">
          <p class="text-center text-xl font-semibold">You don't have any movies added to your seen list!</p>
          <router-link
        ...
        <div v-else>
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <MovieCard v-for="movie in seenMovies" :movie="movie" :showImages="true" :key="movie.id"></MovieCard>
          </div>
        </div>
      </div>
    </section>
...
export default {
  ...
  computed: {
    ...
    seenMovies() {
      ...
      const seenListIds = this.seenList.map((seenItem) => seenItem.movie_id);
